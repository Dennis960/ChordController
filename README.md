...
ChordController is a tool created with pygame and pynput to control the mouse and keyboard with a nintendo switch (or similar) controller.
...
```bash
pip install git+https://github.com/Dennis960/ChordController.git
```
...
- `triple_click`: The button is clicked three times within the configured `double_click_duration`
...
- `MiscellaneousAction`: Actions that do not directly interact with the computer but are part of the software's functionality, such as opening a cheat sheet or switching modes.
