...
from chordcontroller.event_listener import EventListenerConfig, Events
...
class Button(Events[ControllerButtonEventName, "Button"]):
    def __init__(
        self,
        name: ControllerButtonName,
        index: ControllerButtonIndex,
    ...
    def _down(self):
        """Internal function that has to be called when the button is pressed down"""
        self.pressed = True
    ...
    def _up(self):
        """Internal function that has to be called when the button is released"""
        self.pressed = False
...
class Stick(Events[ControllerStickEventName, "Stick"]):
    def __init__(
        self,
        name: str,
        axis_x_index: int,
    ...
    def _move_x(self, value):
        """Internal function that has to be called when the joystick axis changes value. Ignores movements within the deadzone. Does not ignore going into and out of the deadzone."""
    ...
    def direction(
        self, deadzone: float = 0.5
    ) -> Literal["up", "down", "left", "right", "center"]:
        """
        Determine the direction of the joystick based on the current x and y values.
        If both |x| and |y| are less than or equal to deadzone, return "center".
        """
        if abs(self.x) <= deadzone and abs(self.y) <= deadzone:
            return "center"
...
class MultiButtonEvents(
    Events[tuple[ControllerButtonEventName, list[ControllerButtonName]], list[Button]]
):
    registered_buttons: list[ControllerButtonName] | None = None
    """List of buttons that are registered for multi button events even if no event explicitly uses them."""

    def get_event_listeners(self, event_trigger) -> list[EventListenerConfig]:
        event_name, event_button_names = event_trigger
        event_listener_configs: list[EventListenerConfig] = []
        for listener_config in self._event_listener_configs.values():
            event_trigger = listener_config.event_trigger
            if event_trigger is None:
                event_listener_configs.append(listener_config)
                continue
            e_name, button_names = event_trigger
            if e_name == event_name and (
                button_names is None or set(button_names) == set(event_button_names)
            ):
                event_listener_configs.append(listener_config)
        return event_listener_configs

    def add_event_listener(
        self,
        controller_button_event_name: ControllerButtonEventName,
        controller_button_names: list[ControllerButtonName] | None,
        ...
        If the controller_button_names is None, the listener will be called for all events.
        If both controller_button_event_name and controller_button_names are None,
        the listener will be called for all multi button events.
        Note: setting the controller_button_names to None will not register any buttons for multi button events.
        """
        if controller_button_event_name is None and controller_button_names is None:
            return super().add_event_listener(None, listener, tag, single_shot)
        return super().add_event_listener(
            (controller_button_event_name, controller_button_names),
            listener,
            tag,
            single_shot,
        )
...
class Controller(Events[ControllerEvent, None]):
    Stick = Stick
    Button = Button
    ...
    def __init__(self, config: Config):
        ...
        """Is True if the controller is connected and initialized"""
        ...
        self.last_joy_connection_check = 0
        """Time when the controller connection status was last checked in seconds"""

    def remove_all_event_listeners(self, tag: str | None = None):
        for button in self.buttons.values():
            button.remove_all_event_listeners(tag)
        for stick in self.sticks.values():
            stick.remove_all_event_listeners(tag)
        self.multi_button_events.remove_all_event_listeners(tag)
        super().remove_all_event_listeners(tag)
    ...
    def get_buttons_allowed_for_multi_button_event(self) -> list[Button]:
        ...
        if self.multi_button_events.registered_buttons is not None:
            allowed_button_names = self.multi_button_events.registered_buttons
        else:
            allowed_button_names = []
        for (
            listener_config
        ) in self.multi_button_events._event_listener_configs.values():
            if listener_config.event_trigger is None:
                continue
            event_trigger: tuple[
                ControllerButtonEventName, list[ControllerButtonName] | None
            ] = listener_config.event_trigger
            (_, button_names) = event_trigger
    ...
    def on_multi_button_down(self):
        ...
        if (
            ...
            self.multi_button_events.call_event_listeners(
                "triple_click", self.last_multi_button_event_buttons
            )
            self.multi_button_last_double_click_time = 0
            self.multi_button_last_click_time = 0
        elif (
    ...
    def handle_joy_disconnect(self):
        ...
        self.pygame_controller = None
        self.call_event_listeners("controller_disconnected")
    ...
    def handle_joy_connect(self):
        if not self.pygame_controller:
            try:
                ...
                self.pygame_controller = None
                self.call_event_listeners("controller_error")
            if self.pygame_controller:
                print(f"controller connected: {self.pygame_controller.get_name()}")
                self.call_event_listeners("controller_connected")

    def _on_button_down(self, button: Button):
        """Internal function that has to be called when a button is pressed down."""
        button._down()
        if button in self.get_buttons_allowed_for_multi_button_event():
            self.handle_multi_button_down(button)

    def _on_button_up(self, button: Button):
        """Internal function that has to be called when a button is released."""
        button._up()
        if button in self.get_buttons_allowed_for_multi_button_event():
    ...
    def handle_joy_axis_motion(self, event: pygame.event.Event):
        ...
        for stick in self.sticks.values():
            if axis == stick.axis_x_index:
                ...
                stick._move_y(value)
        for button in self.buttons.values():
            if button.index == "axis-4" and axis == 4:
                if value > 0.5:
                    self._on_button_down(button)
                elif value < 0.5 and button.pressed:
                    self._on_button_up(button)
            if button.index == "axis-5" and axis == 5:
                if value > 0.5:
                    self._on_button_down(button)
                elif value < 0.5 and button.pressed:
                    self._on_button_up(button)
    ...
    def update_check_controller_connection(self):
        """
        Call this function periodically to check for controller connection status.
        """
        if not self.pygame_controller:
            now = time.time()
            if now - self.last_joy_connection_check > 1:
                self.handle_joy_connect()
                self.last_joy_connection_check = now
...
if __name__ == "__main__":
    from chordcontroller.config import default_config

    button_event_types: list[ControllerButtonEventName] = [
        "down",
        "up",
        "click",
        "long_press",
